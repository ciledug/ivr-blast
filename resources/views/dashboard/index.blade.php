        ...
        if(data.status){
          ...
          for(let i in data.campaigns){
            ...
            if(data.campaigns.length > 0){
              ...
              detailUrl = '{{ url('campaigns') }}/' + campaign.id;
              rows += '<tr>\
                        <td class="text-end">'+ no +'.</td>\
                        <td>'+ campaign.name +'</td>\
                        <td class="text-end">'+ new Intl.NumberFormat('id-ID').format(campaign.data_called) +'</td>\
                        <td class="text-end">'+ new Intl.NumberFormat('id-ID').format(campaign.data_remaining) +'</td>\
                        <td class="text-end">'+ progress +' %</td>\
                        <td><a href="' + detailUrl + '" class="btn btn-sm btn-primary btn-modal-spinner">Detail</a></td>\
                      </tr>';
            }
          }
